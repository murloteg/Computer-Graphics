...
public class Rotate implements Instrument, DialogEnabled {
    private final ImagePanel imagePanel;
    private final JButton instrumentButton;
    private final JRadioButtonMenuItem menuButton;
    private final List<?> defaultInstrumentParameters = List.of(0);

    public Rotate(ImagePanel imagePanel) {
    ...
    public void injectActionListeners(List<Instrument> instrumentList, ButtonGroup buttonGroup) {
        ...
        instrumentButton.addActionListener(event -> {
            showDialog();
            imagePanel.setFilterMode(FilterMode.ROTATE);
    ...
    public void showDialog() {
        ...
        String angle = String.valueOf(Objects.requireNonNull(parameters.get(0)));
        ...
        JLabel parameterLabel = new JLabel("Choose angle (deg):");
        ...
        JSlider parameterSlider = new JSlider(-180, 180);
        parameterSlider.setValue(Integer.parseInt(angle));
    ...
    private boolean checkIfInputIsValid(String input, JDialog dialog) {
        try {
            ...
            if (value < -180 || value > 180) {
                dialog.dispose();
                JOptionPane.showMessageDialog(null, "Angle should be a value in [-180; 180]", "Incorrect parameter", JOptionPane.ERROR_MESSAGE);
                return false;
            }
