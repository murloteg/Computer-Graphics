...
public class RobertsOperator implements Instrument, DialogEnabled {
    private final ImagePanel imagePanel;
    private final JButton instrumentButton;
    private final JRadioButtonMenuItem menuButton;
    private final List<?> defaultInstrumentParameters = List.of(0.5);

    public RobertsOperator(ImagePanel imagePanel) {
    ...
    public void injectActionListeners(List<Instrument> instrumentList, ButtonGroup buttonGroup) {
        ...
        instrumentButton.addActionListener(event -> {
            showDialog();
            imagePanel.setFilterMode(FilterMode.ROBERTS_OPERATOR);
    ...
    public void showDialog() {
        ...
        String threshold = String.valueOf(Objects.requireNonNull(parameters.get(0)));
        ...
        okButton.addActionListener(event -> {
            ...
            if (checkIfInputIsValid(currentValue, parametersDialog)) {
                imagePanel.addStateToApplicationParameters(getInstrumentName(), List.of(Double.parseDouble(currentValue)));
            }
        ...
        JLabel parameterLabel = new JLabel("Choose threshold:");
    ...
    private boolean checkIfInputIsValid(String input, JDialog dialog) {
        try {
            ...
            if (value < 0 || value > 1) {
                dialog.dispose();
                JOptionPane.showMessageDialog(null, "Threshold should be a value in [0; 1]", "Incorrect parameter", JOptionPane.ERROR_MESSAGE);
                return false;
            }
