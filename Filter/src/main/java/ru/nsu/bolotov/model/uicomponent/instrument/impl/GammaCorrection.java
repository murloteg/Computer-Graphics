...
public class GammaCorrection implements Instrument, DialogEnabled {
    ...
    private final List<?> defaultInstrumentParameters = List.of(1.0);

    public GammaCorrection(ImagePanel imagePanel) {
    ...
    public void injectActionListeners(List<Instrument> instrumentList, ButtonGroup buttonGroup) {
        ...
        instrumentButton.addActionListener(event -> {
            showDialog();
            imagePanel.setFilterMode(FilterMode.GAMMA_CORRECTION);
    ...
    public void showDialog() {
        ...
        String gamma = String.valueOf(Objects.requireNonNull(parameters.get(0)));
        ...
        JLabel parameterLabel = new JLabel("Choose gamma:");
    ...
    private boolean checkIfInputIsValid(String input, JDialog dialog) {
        try {
            ...
            if (value < 0.1 || value > 10.0) {
                dialog.dispose();
                JOptionPane.showMessageDialog(null, "Gamma should be a value in [0.1; 10]", "Incorrect parameter", JOptionPane.ERROR_MESSAGE);
                return false;
            }
